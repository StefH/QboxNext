...
namespace QboxNext.Core.Simulation
{
	...
	{
	    private static readonly Logger Log = LogManager.GetLogger("PatternParser");
		...
		public static List<UsagePatternSpec> ParseSpecList(string inPatternSpec)
		{
			...
			foreach (var part in parts)
			{
				...
				if (spec == null)
					Log.Error(string.Format("Error: could not parse pattern {0}", part));
				else
		...
		private static EUsageShape ParseShapeString(string inShapeString)
		{
			switch (inShapeString)
			{
				...
				default:
					Log.Error(string.Format("Error: unknown shape {0}", inShapeString));
					return EUsageShape.None;
			}
		}
