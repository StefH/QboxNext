...
namespace Qboxes.Model.Classes
{
    /// <summary>
    ...
    public class MiniDataHandler : IVisitor
    {
        ...
        public string Handle()
        {
            using (new ExtendedLogger(_context.Mini.SerialNumber))
            {
                ...
                try
                {
                    Log.Info("sn: {0} | input: {1} | lastUrl: {2}", _context.Mini.SerialNumber, _context.Message, _context.Mini.QboxStatus.Url);
                ...
                catch (Exception e)
                {
                    ...
                    Log.Error(e, $"sn: {_context.Mini.SerialNumber} | Error: {e.Message}");
                    return e.Message;
                }
            }
        }
        ...
        public void Accept(CounterPayload payload)
        {
            try
            {
                ...
                {
                    Log.Debug("Invalid value for counter {0} / {1}", counterPayload.InternalNr, _context.Mini.SerialNumber);
                    return;
                }
                ...
                if (counter == null)
                {
                    Log.Warn($"Recieved value for unknown counter: {payload.InternalNr} / {_context.Mini.SerialNumber}");
