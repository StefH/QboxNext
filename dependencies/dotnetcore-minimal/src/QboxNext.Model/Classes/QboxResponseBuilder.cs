...
namespace QboxNext.Model.Classes
{
    ...
    public class QboxResponseBuilder
    {
        ...
        /// Constructor.
        /// </summary>
        public QboxResponseBuilder(Mini inMini, IQueue<string> ioCommandQueue, bool inCanSendAutoStatusCommands, bool inCanSendAutoAnswerCommand)
        {
            _mini = inMini;
        ...
        private string GetQueueCommand()
        {
            ...
            try
            {
                ...
                Logger.LogInformation("{0} DeQueue Command: {1}", _mini.SerialNumber, command ?? "null");
                return command;
            }
            catch (Exception e)
            {
                ...
                if (ClientAutoCommandsAllowed(clientState))
                {
                    ...
                    else
                    {
                        if (clientState != null)
                        {
                            ...
                            {
                                Logger.LogWarning("Actions failed on client {0} of Qbox {1}", client.ClientId, _mini.SerialNumber);
        ...
        private static readonly ILogger Logger = QboxNextLogProvider.CreateLogger("QboxResponseBuilder");
        private Mini _mini;
